...
const Sparkle = ({
  ...
  ease: 'easeOut'
    ...
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className="text-amber-300 drop-shadow-[0_0_6px_rgba(251,191,36,0.8)]">
...
const Confetti = ({
  ...
  color: string;
  left: string;
}) => <motion.div className="absolute w-2 h-3 rounded-sm pointer-events-none" style={{
  left,
...
export const PongalOfferBanners = () => {
  ...
  return <section className="container px-4 py-8">
      <div className="relative overflow-hidden rounded-2xl shadow-lg h-56 sm:h-64 md:h-72">
        ...
        {/* Falling Confetti */}
        {confettiPieces.map(piece => <Confetti key={piece.id} {...piece} />)}
        ...
        <div className="absolute bottom-3 right-4 flex gap-2 z-10">
          {banners.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-amber-400 w-6' : 'bg-white/50 hover:bg-white/80'}`} />)}
        </div>
      </div>
    </section>;
};
