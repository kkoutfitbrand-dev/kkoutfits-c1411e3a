...
const Firework = ({
  ...
  return <motion.div className="absolute pointer-events-none" style={{
    ...
    repeatDelay: 3
      ...
      {particles.map((particle, i) => <motion.div key={i} className="absolute w-2 h-2 rounded-full" style={{
      ...
      <motion.div className="absolute w-3 h-3 rounded-full -translate-x-1/2 -translate-y-1/2" style={{
      backgroundColor: color,
...
const InteractiveFirework = ({
  ...
  return <motion.div className="absolute pointer-events-none z-50" style={{
    left: x,
    top: y
  }} initial={{
    opacity: 1
    ...
    duration: 1.5
      ...
      {particles.map((particle, i) => <motion.div key={i} className="absolute w-2.5 h-2.5 rounded-full" style={{
      backgroundColor: color,
...
const CountdownTimer = () => {
  ...
  return <div className="flex items-center justify-center gap-2 sm:gap-3 md:gap-4 lg:gap-6">
...
export const NewYearBanner = () => {
  ...
  const [interactiveFireworks, setInteractiveFireworks] = useState<{
    id: number;
    x: number;
    y: number;
  }[]>([]);
  ...
  return <section ref={ref} className="relative overflow-hidden" onClick={handleInteraction}>
      <div ref={containerRef} className="relative min-h-[600px] md:min-h-[700px] py-16 md:py-24" style={{
      background: 'linear-gradient(135deg, #0c1929 0%, #1a1a2e 25%, #16213e 50%, #0f0f23 75%, #1a0a2e 100%)'
        ...
        <div className="absolute inset-0 overflow-hidden">
          {Array.from({
          length: 50
        }).map((_, i) => <motion.div key={i} className="absolute rounded-full bg-white" style={{
        ...
        <div className="absolute bottom-0 right-1/4 w-80 h-80 bg-amber-500/15 rounded-full blur-[80px] animate-pulse" style={{
        animationDelay: '1s'
      }} />
        ...
        {interactiveFireworks.map(fw => <InteractiveFirework key={fw.id} x={fw.x} y={fw.y} />)}
          ...
          <div className="flex flex-col items-center text-center">
            ...
            delay: 0.7
              ...
              <Link to="/new-year-collection">
                <Button size="lg" className="group relative overflow-hidden bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-black font-bold px-8 py-6 text-lg rounded-full shadow-[0_0_30px_rgba(251,191,36,0.4)] transition-all duration-300 hover:shadow-[0_0_50px_rgba(251,191,36,0.6)]" onMouseEnter={handleInteraction}>
