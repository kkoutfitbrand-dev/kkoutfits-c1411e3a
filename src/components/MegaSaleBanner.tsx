...
const DiscountBadge = ({
  ...
  percent: number;
  className?: string;
}) => <motion.div className={`relative ${className}`} animate={{
  scale: [1, 1.05, 1],
  rotate: [0, 2, -2, 0]
}} transition={{
  duration: 3,
  ...
  ease: 'easeInOut'
    ...
    <div className="relative">
      <motion.div className="absolute inset-0 rounded-full blur-xl bg-primary/30" animate={{
      scale: [1, 1.3, 1],
      opacity: [0.3, 0.6, 0.3]
    }} transition={{
      duration: 2,
...
// Shimmer effect
const ShimmerLine = () => <motion.div className="absolute inset-0 overflow-hidden" initial={{
  x: '-100%'
}} animate={{
  x: '200%'
}} transition={{
...
export const MegaSaleBanner = () => {
  ...
  return <section ref={ref} className="relative overflow-hidden">
      <div className="relative min-h-[380px] sm:min-h-[420px] md:min-h-[480px]">
        ...
        {/* Animated top border */}
        <motion.div className="absolute top-0 left-0 right-0 h-1" style={{
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10 flex items-center min-h-[380px] sm:min-h-[420px] md:min-h-[480px]">
          <div className="max-w-2xl py-8 sm:py-10">
            ...
            delay: 0.5
              ...
              {['Free Shipping', 'Easy Returns', 'Best Prices'].map((feature, index) => <motion.div key={feature} initial={{
              opacity: 0,
              scale: 0.8
            }} animate={isInView ? {
              opacity: 1,
            ...
            delay: 0.7
              ...
              <Link to="/shop">
                <Button size="lg" className="group relative overflow-hidden bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-6 sm:px-8 py-5 sm:py-6 text-sm sm:text-base rounded-full transition-all duration-300 hover:scale-105 shadow-xl hover:shadow-2xl w-full sm:w-auto">
                  <ShimmerLine />
              ...
              <Link to="/trending">
                <Button size="lg" variant="outline" className="font-semibold px-6 sm:px-8 py-5 sm:py-6 text-sm sm:text-base rounded-full border-2 border-border hover:border-primary hover:bg-primary/5 transition-all duration-300 w-full sm:w-auto">
                  View Trending
                </Button>
              </Link>
            </motion.div>
