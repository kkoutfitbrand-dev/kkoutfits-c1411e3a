import { motion } from 'framer-motion';
import { Sun, Sparkles, ShoppingBag, Clock } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { useSummerCountdown } from '@/hooks/useSummerCountdown';

export const SummerSaleBanner = () => {
  const { days, hours, minutes, seconds, status } = useSummerCountdown();

  if (status === 'ended') return null;

  const showCountdown = status === 'countdown';

  return (
    <section className="relative overflow-hidden">
      <motion.div
        className="absolute inset-0"
        style={{
          background: 'linear-gradient(135deg, hsl(200 80% 50%) 0%, hsl(200 70% 55%) 40%, hsl(38 90% 55%) 100%)',
          backgroundSize: '200% 200%',
        }}
        animate={{ backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'] }}
        transition={{ duration: 6, repeat: Infinity }}
      />

      {/* Floating sun rays */}
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(6)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute"
            style={{ left: `${10 + i * 16}%`, top: '50%' }}
            animate={{
              y: [-15, 15, -15],
              opacity: [0.15, 0.3, 0.15],
              scale: [0.9, 1.1, 0.9],
            }}
            transition={{ duration: 3 + i * 0.5, repeat: Infinity, delay: i * 0.3 }}
          >
            <Sun className="w-5 h-5 text-white/25" />
          </motion.div>
        ))}
      </div>

      <div className="container px-4 py-8 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <motion.div
              animate={{ scale: [1, 1.1, 1], rotate: [0, 10, -10, 0] }}
              transition={{ duration: 3, repeat: Infinity }}
              className="w-14 h-14 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center"
            >
              <ShoppingBag className="w-7 h-7 text-white" />
            </motion.div>
            <div>
              <motion.p
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                className="text-white/90 text-sm font-medium flex items-center gap-2"
              >
                <Sparkles className="w-4 h-4" />
                Summer Collection 2026
              </motion.p>
              <h3 className="text-xl md:text-2xl font-bold text-white">
                {showCountdown ? 'Summer Sale — Up to 50% Off!' : 'Coming Soon!'}
              </h3>
            </div>
          </div>

          {showCountdown && (
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              className="flex items-center gap-2 md:gap-4"
            >
              <Clock className="w-5 h-5 text-white/80 hidden md:block" />
              {[
                { value: days, label: 'Days' },
                { value: hours, label: 'Hrs' },
                { value: minutes, label: 'Min' },
                { value: seconds, label: 'Sec' },
              ].map((item, index) => (
                <motion.div
                  key={item.label}
                  className="flex flex-col items-center"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: index * 0.1 }}
                >
                  <motion.div
                    className="w-14 h-14 md:w-16 md:h-16 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center"
                    animate={{ scale: item.label === 'Sec' ? [1, 1.05, 1] : 1 }}
                    transition={{ duration: 1, repeat: Infinity }}
                  >
                    <span className="text-2xl md:text-3xl font-bold text-white">
                      {String(item.value).padStart(2, '0')}
                    </span>
                  </motion.div>
                  <span className="text-xs text-white/80 mt-1">{item.label}</span>
                </motion.div>
              ))}
            </motion.div>
          )}

          <Link to="/shop">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                size="lg"
                className="bg-white text-sky-700 hover:bg-white/90 font-bold shadow-xl"
              >
                <ShoppingBag className="w-5 h-5 mr-2" />
                Shop Summer
              </Button>
            </motion.div>
          </Link>
        </div>
      </div>
    </section>
  );
};
