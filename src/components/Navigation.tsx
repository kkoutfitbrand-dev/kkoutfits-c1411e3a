  ...
  return <header className={`w-full border-b border-border bg-background/95 backdrop-blur-md sticky top-0 z-50 transition-all duration-500 ${isScrolled ? 'shadow-lg py-0' : 'py-1'}`}>
      ...
      <div className={`container flex items-center justify-between px-4 transition-all duration-500 ${isScrolled ? 'h-12' : 'h-16 md:h-20'}`}>
        ...
        <Link to="/" className="flex items-center">
          <h1 className="text-xl md:text-2xl lg:text-3xl font-medium tracking-tight text-foreground">KKOutfits</h1>
        </Link>

        {/* Desktop Navigation */}
        <nav className="hidden lg:flex items-center gap-6">
          {categories.slice(0, 5).map(category => (
            <Link key={category} to={`/category/${category.toLowerCase().replace(/\s+/g, '-')}`} className="text-sm font-medium text-foreground/80 hover:text-foreground transition-colors">
              {category}
            </Link>
          ))}
        </nav>
