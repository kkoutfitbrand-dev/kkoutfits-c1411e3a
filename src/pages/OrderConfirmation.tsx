            ...
            <div>
              ...
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>₹{(orderDetails.total ?? 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Shipping</span>
                  <span className={`font-medium ${(orderDetails.total ?? 0) > 999 ? 'text-green-600' : ''}`}>
                    {(orderDetails.total ?? 0) > 999 ? "FREE" : "₹99"}
                  </span>
                </div>
                <Separator className="my-2" />
                <div className="flex justify-between font-bold text-base">
                  <span>Total</span>
                  <span>₹{((orderDetails.total ?? 0) + ((orderDetails.total ?? 0) > 999 ? 0 : 99)).toLocaleString()}</span>
                </div>
              </div>
