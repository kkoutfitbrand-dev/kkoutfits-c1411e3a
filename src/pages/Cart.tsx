...
const Cart = () => {
  ...
  const saveCartToDb = async (items: CartItem[]) => {
    ...
    try {
      const {
        data: existingCart
      } = await supabase.from('carts').select('id').eq('user_id', user.id).maybeSingle();
      if (existingCart) {
  ...
  const updateQuantity = async (id: string, delta: number) => {
    const newItems = cartItems.map(item => item.id === id ? {
      ...item,
        ...
        <div className="container px-4 py-6 md:py-12">
          ...
          <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2 space-y-3 md:space-y-4">
              {cartItems.map(item => <div key={item.id} className="bg-card border border-border rounded-lg p-3 md:p-4 flex gap-3 md:gap-4">
                  <img src={item.image} alt={item.name} className="w-20 h-20 md:w-32 md:h-32 object-cover rounded-md flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <h3 className="font-semibold text-sm md:text-base line-clamp-2">{item.name}</h3>
                        {item.is_combo && <span className="inline-block bg-primary/10 text-primary text-xs px-2 py-0.5 rounded-full mt-1">
                            Combo Bundle
                          </span>}
                      </div>
                    ...
                    {item.is_combo ? <div className="mt-2">
                        ...
                        <div className="flex flex-wrap gap-1.5 md:gap-2 mt-1.5">
                          {item.combo_items && item.combo_items.length > 0 ? item.combo_items.map((ci, idx) => <div key={idx} className="flex items-center gap-1.5 bg-muted/50 rounded-md px-1.5 py-1 md:px-2 md:py-1.5">
                                {ci.image_url ? <img src={ci.image_url} alt={ci.color_name || `Item ${idx + 1}`} className="w-6 h-6 md:w-8 md:h-8 rounded object-cover border border-border" /> : <div className="w-6 h-6 md:w-8 md:h-8 rounded bg-muted border border-border flex items-center justify-center">
                              ...
                              </div>) : item.color ? <span className="text-xs md:text-sm text-muted-foreground">{item.color}</span> : <span className="text-xs md:text-sm text-muted-foreground">{item.size || 'Bundle items'}</span>}
                        </div>
                      </div> : <div className="flex flex-wrap gap-x-2 md:gap-x-3 text-xs md:text-sm text-muted-foreground mt-1 md:mt-2">
                    ...
                    <div className="flex items-center justify-between mt-2 md:mt-4">
                      ...
                      {!item.is_combo ? <div className="flex items-center border border-border rounded-md">
                          <button onClick={() => updateQuantity(item.id, -1)} className="p-1.5 md:p-2 hover:bg-muted transition-colors">
                        ...
                        </div> : <span className="text-xs md:text-sm text-muted-foreground">
          ...
          {savedItems.length > 0 && <div className="mt-8">
              <SavedForLater items={savedItems} onMoveToCart={moveToCart} onRemove={removeSavedItem} />
            </div>}
        </div>
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-card/95 backdrop-blur-md border-t border-border p-4 lg:hidden z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
          <div className="space-y-3">
            ...
            {!appliedCoupon ? <div className="flex gap-2">
                <Input placeholder="Coupon code" value={couponCode} onChange={e => {
              setCouponCode(e.target.value.toUpperCase());
              setCouponError(null);
              ...
              </div> : <div className="flex items-center justify-between bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg p-2">
                <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 text-green-600" onClick={removeCoupon}>
                  <X className="w-3 h-3" />
                </Button>
              </div>}
            {couponError && <p className="text-xs text-destructive">{couponError}</p>}
