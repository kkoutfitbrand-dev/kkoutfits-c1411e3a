...
const ComboDetail = () => {
  ...
  const toggleColorSelection = (item: ComboItem) => {
    ...
    if (existing) {
      ...
      setSelectedColors([...selectedColors, { item, quantity: 1 }]);
    }
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 mb-4">
                    {selectedColors.map(sc => (
                      <div key={sc.item.id} className="flex justify-between text-sm">
                        <span>{sc.item.color_name} × {sc.quantity}</span>
                        <span>₹{((combo.combo_price_cents / 100) * sc.quantity).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                  <div className="border-t pt-3 flex justify-between font-bold">
                    ...
                    <div className="text-right">
                      ...
                      {getOriginalTotal() > getTotalPrice() && (
